...
if(init_usrcfg == "no"){
    ...
    OvmsConfig.Set("usr", "gps.ticker", "60");     	    // ticker format 1/10/60/300/600/3600 seconds
    ...
    OvmsConfig.Set("usr", "gps.pubsub", "0");       // PubSub ID
}

var gps = {
    init: false,
    counter: 0,
    pubsub: OvmsConfig.Get("usr","gps.pubsub", "0"),
};

// delete old events
PubSub.unsubscribe(gps.pubsub);
    ...
    if(gps_power_switch == "yes"){
        ...
        if(!veh_on() && !charging()) {        
            ...
            if( me_parktime >= gps_parktime ){
                gps.counter = gps.counter +1;
                if( (gps_on == "no")&&(gps.counter >= gps_counter_value) ){
                    OvmsCommand.Exec('cellular gps start');
                    OvmsConfig.Set("usr", "gps.on", "yes");
                }else{
                    if( (gps_on == "yes")&&(gps.counter >= (gps_counter_value + gps_counter_add)) ){
...
if(!gps.init){
    gps.init = true;
    gps.pubsub = PubSub.subscribe(gps_ticker,usr_gps_check);
    OvmsConfig.Set("usr", "gps.pubsub", gps.pubsub);
}
